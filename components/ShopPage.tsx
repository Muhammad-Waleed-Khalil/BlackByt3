      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {shopItems.map((item, i) => (
            <div key={i} className="bg-black border border-gray-700 p-6 hover:border-red-600 transition-all group flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-start mb-4">
                  <item.icon className="text-red-900 group-hover:text-red-500 transition-colors w-6 h-6" />
                  <span className="text-xs font-mono border border-red-900/30 px-2 py-1 text-gray-400 uppercase">{item.type}</span>
                </div>
                <h4 className="text-xl font-['Unica_One'] text-white mb-2">{item.title}</h4>
                <p className="text-sm font-mono text-gray-300 mb-4">{item.description}</p>
                <p className="text-xs font-mono text-gray-400 mb-6">{item.details}</p>
              </div>
